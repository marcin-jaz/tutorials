...
<SCRIPT SRC="/wcs/javascript/tools/adminconsole/RoleDialog.js"></SCRIPT>
<SCRIPT>
...
%>
     if (isEmpty(document.wizard1.name.value)) {
     	alertDialog("<%=UIUtil.toJavaScript((String)rolesDialogNLS.get("memberGroupGeneralNameEmpty"))%>");
     	return false;
     }
     ...
     for (var i=0; i < <%=numberOfRoles%>;i++) {
     	if (trim(rolename) == RoleArray[i].name) {
     	    alertDialog("<%=UIUtil.toJavaScript((String)rolesDialogNLS.get("roleExists"))%>");
     	    return false;
     	}
     }
...
%> 
<LABEL>
<%=UIUtil.toHTML((String)rolesDialogNLS.get("memberGroupGeneralName"))%>: <i><%= UIUtil.toHTML(roleBean.getName()) %></i>
</LABEL>
<% } %>
...
<TEXTAREA name="desc" rows="7" cols="63" id="desc1"></TEXTAREA>
<BR>
</LABEL>
<% } else {
%> 
<LABEL for="desc2">
<%=UIUtil.toHTML((String)rolesDialogNLS.get("memberGroupGeneralDesc"))%><BR>
<TEXTAREA name="desc" rows="7" cols="63" id="desc2"><%= UIUtil.toHTML(desc) %></TEXTAREA>
<BR>
</LABEL>
<% } %>
</TD></TR>
