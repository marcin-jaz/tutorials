...
<%@ page import="com.ibm.commerce.member.helpers.ECMemberConstants" %>
...
<%@include file="../common/common.jsp" %>
      ...
      try {
   	...
   	rootRoles = dbRole.getAllRoles();
   	numberOfRoles = rootRoles.length;
   	roleNames = new String[numberOfRoles][3];
	...
	function getUserNLSTitle(){
	  return "<%=rolesNLS.get("roles")%>";
        ...
        function newRole(){
      	    var url = top.getWebappPath() + "DialogView?XMLFile=buyerconsole.BuyRolesDialog";
      	    if (top.setContent){
        ...
        if (checked.length > 0){
          ...
          for (var x = 0; x < <%=numberOfRoles%>; x++) {
             if (RoleArray[x].name == checked[0]) roleId = RoleArray[x].roleId;
      	...
      	if (changed != 0)
      	{
        var url = "<%="/webapp/wcs/tools/servlet/DialogView?XMLFile=adminconsole.RolesDialog"%>";
        url += "&roleId=" + roleId;
        if (top.setContent){
          top.setContent("<%= UIUtil.toJavaScript((String)rolesNLS.get("roleEdit")) %>",
                         url,
                         true);
        }
        else{
      ...
      <FORM NAME="rolesForm">
        ...
        <%= comm.endDlistRow() %>
          ...
          %>
        	...
        	<%= comm.addDlistColumn( UIUtil.toHTML(roleNames[i][1]),"none") %>
        	<%= comm.addDlistColumn( UIUtil.toHTML(roleNames[i][2]),"none") %>
        	<%= comm.endDlistRow() %>
        	<%
          }
