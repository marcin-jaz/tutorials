...
<%! 
	...
	public String getPaymentStatus(String orderId, Integer storeId, Hashtable tmpOrderLabels, HttpServletRequest request) {
		String paymentState = null;
		
		PayStatusPMDataBean payStatusBean = new PayStatusPMDataBean();
//		payStatusBean.setOrderId(orderId);
//		payStatusBean.setStoreId(storeId.toString());
		try {
	...
	public String getPONumber(OrderDataBean odb) {
		String poNumber = "";
		try {
			poNumber = odb.getPurchaseOrderNumber();   			
		} catch (Exception ex) {
			//Exception
	...
	public String getAVSCodeMsg(Hashtable tmpOrderLabels, HttpServletRequest request) {
		...
		try {
			...
			//Exception
			System.out.println("Exception in OrderSummaryDetails.jsp (getAVSCodeMsg)");	
			ex.printStackTrace();
			avsCode = null;
		}
...
<HTML>
  ...
  <BODY CLASS=content ONLOAD="parent.setContentFrameLoaded(true);">        
	...
	<!-- Order item details information -->
		...
		<%
			String PONum = getPONumber(orderBean);
	...
	<table width=95%>
		...
		<tr>
		    ...
		    <table cellpadding=0 cellspacing=0 border=0>      
		        	...
		        	<td></td>
				<td></td>
				<td colspan=2 align=right nowrap><%= UIUtil.toHTML( (String)orderMgmtNLS.get("orderLevelDiscount")) %></td>
				<td align=right width=130>
		        	...
		        	<td></td>
				<td></td>
				<td colspan=2 align=right nowrap><%= UIUtil.toHTML( (String)orderMgmtNLS.get("minusAdjustment")) %></td>
				<td align=right width=130>
					<script>
