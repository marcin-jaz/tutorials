...
<jsp:setProperty property="hitsPerPage" name="approvalStatusListBean" value="100" />
...
<%
   ...
   // in case the records is from search result, need to double check if there is related reseller approval 
   // record related to orgnization ones
   if(searchFlowType != null && !searchFlowType.equals(""))
   {
	   com.ibm.commerce.ubf.objects.FlowTypeAccessBean flowTypeAB = new com.ibm.commerce.ubf.objects.FlowTypeAccessBean();
	   
	   flowTypeAB.setInitKey_id(Long.valueOf(searchFlowType));
	   flowTypeAB.refreshCopyHelper();
	   String searchFlowTypeDesc = flowTypeAB.getIdentifier();

	   if(ECUserConstants.EC_ORG_APPROVE_FLOWDESC.equals(searchFlowTypeDesc)){
		  //re-initialize the flow type access bean with reseller type
		  flowTypeAB = new com.ibm.commerce.ubf.objects.FlowTypeAccessBean().findFlowTypeByIdentifier(ECUserConstants.EC_USER_APPROVE_FLOWDESC);
		  
	      searchFlowType = flowTypeAB.getId();
	      approvalStatusListBean.setFlowTypeId(searchFlowType);
	      com.ibm.commerce.beans.DataBeanManager.activate(approvalStatusListBean, request);
	      bList = approvalStatusListBean.getApprovalStatusBeans();
	   }
   }
