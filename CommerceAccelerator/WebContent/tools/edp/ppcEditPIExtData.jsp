...
<script language="JavaScript">
	...
	var extendDataArray=new Array();
	var protocolDataArray=new Array();
...
function restorePlainExtendDataBeforeSave(){
	 for (i=0;i<ListDatas.data.length;i++) {
	 	var key=ListDatas.data[i].key;
		var value= ListDatas.data[i].value;
		if(protocolDataArray[i]!=null){
			var valueInProtocolData = protocolDataArray[i].value;
			var valueInExtendData = extendDataArray[i].value;
			if(value == valueInProtocolData) {
			     ListDatas.data[i].value = valueInExtendData;
		    }
		}
	 }
}
...
function savePanelData(){
	restorePlainExtendDataBeforeSave();
...
<%=UIUtil.toHTML((String) ppcLabels.get("editPIExtendedData"))%></td>
</TR>
</TBODY>
</TABLE>
<SCRIPT>
...
%>
	...
	if(back !="back"){	
		ListDatas.data[<%=index%>]=new extData('<%=key%>','<%=UIUtil.toJavaScript(UIUtil.toHTML(value))%>','<%=type%>');
		extendDataArray[<%=index%>] = new extData('<%=key%>','<%=UIUtil.toJavaScript(UIUtil.toHTML((hashData.get(key)).toString()))%>','<%=type%>');
		<%
		if(protocolData.get(key) != null){
		%>
			protocolDataArray[<%=index%>] = new extData('<%=key%>','<%=UIUtil.toJavaScript(UIUtil.toHTML((protocolData.get(key)).toString()))%>','<%=type%>');
		<%  
		}else{
		%>
			protocolDataArray[<%=index%>] = null;
		<%
		}
		%>
	}
