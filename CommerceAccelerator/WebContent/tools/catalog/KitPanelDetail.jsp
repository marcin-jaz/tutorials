...
<%@page import="com.ibm.commerce.catalog.beans.CatalogEntryConfigurationDataBean" %>
...
<%
   try {
    ...
    String strConfiguratorUrl="";
    boolean isNotBundle = false ;
    ...
    if ( productRefNum != null && !strCatentryType.equals("BundleBean"))
    {
    	isNotBundle = true;
    }
    ...
    //Retrieve configurator url for dynamic kits
    try{
	    CatalogEntryConfigurationDataBean dynamicKitConfig = new CatalogEntryConfigurationDataBean();
	    if(productRefNum != null && strCatentryType.equalsIgnoreCase("DynamicKitBean")){    	
	    	dynamicKitConfig.setDataBeanKeyCatalogEntryId(productRefNum);
	    	DataBeanManager.activate(dynamicKitConfig, request);
	    	strConfiguratorUrl=dynamicKitConfig.getUrl();
	    }
	}catch(Exception ex){
		ex.toString();//:(
		// if the url was not created before, there's an exception here
	}
...
if (<%=isNotBundle %>)
{
	parent.setPanelAttribute("Fulfillment","hasTab","YES");
	parent.setPanelAttribute("UnitOfMeasure","next","Fulfillment");  
	parent.setPanelAttribute("Advanced","prev","Fulfillment"); 
	parent.TABS.location.reload();
}
...
function PropertyData(type, sku, name, configuratorUrl, published, buyable,
...
function Property(type, sku, name, configuratorUrl, published, buyable,
                  startYear, startMonth, startDay,
                  endYear, endMonth, endDay,
                  availabilityYear, availabilityMonth, availabilityDay,
                  lastOrderYear, lastOrderMonth, lastOrderDay,
                  ...
                  discontinueYear, discontinueMonth, discontinueDay) 
{
    this.data = new PropertyData(type, sku, name, configuratorUrl, published, buyable,
                                 startYear, startMonth, startDay,
                                 endYear, endMonth, endDay,
		                         availabilityYear, availabilityMonth, availabilityDay,
                	             lastOrderYear, lastOrderMonth, lastOrderDay,
...
function load()
{
    ...
    this.data.configuratorUrl = parent.cutspace(this.formref.configuratorUrl.value);	
...
function display()
{
	...
	//type
	if(!((this.data.type=="PackageBean")||(this.data.type=="BundleBean")||(this.data.type=="DynamicKitBean")))
		this.data.type="PackageBean";
	...
	<% if(productRefNum != null) {
			if (strCatentryType.equalsIgnoreCase("DynamicKitBean"))
			{
%>
				configurator1.style.display = "block";
				configurator2.style.display = "block";
<%
			}
	%>
		for(var i=0; i<this.formref.kit_type.length; i++)
			this.formref.kit_type[i].disabled=true;
	<% } else { %> wizardRow.style.display="block"; <% } %>
	
	// Enable/disable Configurator URL entry field
	this.formref.configuratorUrl.value = this.data.configuratorUrl;
	if (this.data.type=="DynamicKitBean")
	{
	   this.formref.configuratorUrl.disabled=false;
	   configurator1.style.display = "block";
	   configurator2.style.display = "block";
	...
	{
	   this.formref.configuratorUrl.disabled=true;
	}
...
function savePanelData()
{
   ...
   if (prop.getData().type=="BundleBean")
   {
      parent.setPanelAttribute("Fulfillment","hasTab","NO");
      parent.setPanelAttribute("UnitOfMeasure","next","Advanced");
      parent.setPanelAttribute("Advanced","prev","UnitOfMeasure");
   }
   else
   {
      parent.setPanelAttribute("Fulfillment","hasTab","YES");
...
function initForm(){
    ...
    if (parent.get("productSkuRequiredMessage", false))
       {
        ...
        alertDialog("<%= UIUtil.toJavaScript((String)productResource.get("kitSkuRequiredMessage"))%>");
       }
    ...
    if (parent.get("productNameRequiredMessage", false))
       {
        ...
        alertDialog("<%= UIUtil.toJavaScript((String)productResource.get("kitNameRequiredMessage"))%>");
       }
       
    if (parent.get("configuratorURLNameRequiredMessage", false))
           {
            parent.remove("productNameRequiredMessage");
    		document.details.name.select();
            alertDialog("<%= UIUtil.toJavaScript((String)productResource.get("configuratorUrlRequiredMessage"))%>");
       }
    ...
    if (parent.get("productNameNotValidMessage", false))
       {
		...
		document.details.name.select();
        alertDialog("<%= UIUtil.toJavaScript((String)productResource.get("kitNameNotValidMessage"))%>");
       }
    ...
    if (parent.get("fieldSizeExceeded_configuratorUrl", false))
       {
        parent.remove("fieldSizeExceeded_configuratorUrl");
	document.details.configuratorUrl.select();
        alertDialog("<%= UIUtil.toJavaScript((String)productResource.get("fieldSizeExceeded"))%>");

       }

    if (parent.get("configuratorUrlSpace_url", false))
       {
        parent.remove("configuratorUrlSpace_url");
	document.details.configuratorUrl.select();
        alertDialog("<%= UIUtil.toJavaScript((String)productResource.get("configuratorUrlSpace"))%>");
...
function validatePanelData(){
    ...
    var objUrl = document.details.configuratorUrl.value;
    ...
    if(!document.details.configuratorUrl.value && !document.details.configuratorUrl.disabled){
	document.details.configuratorUrl.select();
	alertDialog("<%=UIUtil.toJavaScript((String)productResource.get("configuratorUrlRequiredMessage"))%>");
	return false;
    }

    if ( !isValidUTF8length(objUrl, 254)  ){
	document.details.configuratorUrl.select();
	alertDialog("<%=UIUtil.toJavaScript((String)productResource.get("fieldSizeExceeded"))%>");     	
   	return false;
    }

    if (numOfOccur(objUrl, ' ') > 0) {
	document.details.configuratorUrl.select();
	alertDialog("<%=UIUtil.toJavaScript((String)productResource.get("configuratorUrlSpace"))%>");     	
   	return false;
    }
...
function needConfiguratorURL(isDynamicKit)
{
   if (isDynamicKit==true)
   {
      configurator1.style.display="block";
      configurator2.style.display="block";
      document.details.configuratorUrl.disabled=false;
   }
   else
   {
      configurator1.style.display="none";
      configurator2.style.display="none";
      document.details.configuratorUrl.disabled=true;
   }
}
...
<%
	if(productRefNum != null) {
		if (strCatentryType.equalsIgnoreCase("PackageBean")) {  %> <%=UIUtil.toHTML((String)productResource.get("attribute_type"))%>: <I><%=UIUtil.toHTML((String)productResource.get("attribute_catentryType_package"))%></I><BR><BR> <% }
		else if (strCatentryType.equalsIgnoreCase("BundleBean")) { %> <%=UIUtil.toHTML((String)productResource.get("attribute_type"))%>: <I><%=UIUtil.toHTML((String)productResource.get("attribute_catentryType_bundle"))%></I><BR><BR> <% }
		else if (strCatentryType.equalsIgnoreCase("DynamicKitBean")) { %> <%=UIUtil.toHTML((String)productResource.get("attribute_type"))%>: <I><%=UIUtil.toHTML((String)productResource.get("attribute_catentryType_dynKit"))%></I><BR><BR> <% }
	} else {
%>
		<%=UIUtil.toHTML((String)productResource.get("attribute_type"))%>
<%
	}
%>
           </TD>
    </TR>
    <TR ID="wizardRow" STYLE="display:none">
       <TD colspan="5">

		<TABLE width="100%">
		   <TR>
		      <TD> 
			<LABEL for="PackageBeanID"><input type="radio" name="kit_type" id="PackageBeanID" value="PackageBean" checked onClick="needConfiguratorURL(false)"> <%=UIUtil.toHTML((String)productResource.get("attribute_catentryType_package"))%></LABEL>&nbsp;&nbsp;
		      </TD>
			   </TR>
			   <TR>
			      <TD>   
		         <LABEL for="BundleBeanID"><input type="radio" name="kit_type" id="BundleBeanID" value="BundleBean" onClick="needConfiguratorURL(false)"> <%=UIUtil.toHTML((String)productResource.get("attribute_catentryType_bundle"))%></LABEL>&nbsp;&nbsp;
		      </TD>
		   </TR>
		   <TR>
		      <TD>   
		         <LABEL for="DynamicKitBeanID"><input type="radio" name="kit_type" id="DynamicKitBeanID" value="DynamicKitBean" onClick="needConfiguratorURL(true)"> <%=UIUtil.toHTML((String)productResource.get("attribute_catentryType_dynKit"))%></LABEL>&nbsp;&nbsp;
		      </TD>
		   </TR>
		</TABLE>   
			<BR>       
    ...
    <TR>
           <TD colspan="4">
               <LABEL for="codeID"><%=UIUtil.toHTML((String)productResource.get("part_number"))%></LABEL>
           </TD>
    </TR>
    <TR>
           <TD colspan="4">
    ...
    <TR>
           <TD colspan="4">
               <LABEL for="nameID"><%=UIUtil.toHTML((String)productResource.get("product_name"))%></LABEL>
           </TD>
    </TR>
    <TR>
           <TD colspan="4">
    ...
    <TR ID=configurator1 style="display:none">
           <TD colspan="4">
               <LABEL for="configurator_urlID"><%=UIUtil.toHTML((String)productResource.get("configurator_url"))%></LABEL>
           </TD>
    </TR>
    <TR ID=configurator2 style="display:none">
           <TD colspan="4">
           <INPUT id="configurator_urlID" size="58" maxlength="254" type="text" name="configuratorUrl" disabled><BR><BR>
       </TD>
    </TR>
    ...
    <TR>
           <TD colspan="4">
               <LABEL for="publishedID"><INPUT type="checkbox" id="publishedID" name="published"> &nbsp;<%=UIUtil.toHTML((String)productResource.get("published"))%></LABEL><BR>
       </TD>
    </TR>
    <TR>
           <TD colspan="4">
               <LABEL for="buyableID"><INPUT type="checkbox" id="buyableID" name="buyable" onclick=checkAuction()> &nbsp;<%=UIUtil.toHTML((String)productResource.get("buyable"))%></LABEL><BR>
       </TD>
    </TR>
