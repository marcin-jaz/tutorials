...
/*******
	...
	while (e.hasMoreElements()) {
		...
		if ((catlogEntryAccBean.getType().equals("ProductBean"))) {
			catalogEntryID = catlogEntryAccBean.getCatalogEntryReferenceNumberInEJBType();
...
******/
	...
	while(e.hasMoreElements()){
		...
		{
			catalogEntryID = abCatentry.getCatalogEntryReferenceNumberInEJBType();
			catEntryIDList.add(catalogEntryID);
		}
	}
	...
	for(int i=0; i<catalogGroupIDList.size(); i++) {
		...
		catgrpaccbean.refreshCopyHelper();
	...
	if(catGrpID != null) {
		...
		while (egrp.hasMoreElements()) {
			...
			if(abCatentry.getType().equals("ProductBean"))
			{
				...
				catEntryDescAccBean.refreshCopyHelper();
