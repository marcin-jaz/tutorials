...
for (int i=0; i < numberOfShips ; i++)
{
      ...
      com.ibm.commerce.fulfillment.objects.ShippingModeDescriptionAccessBean shipModeDesc 
      	= shipmode.getDescription(langId,shipmode.getStoreEntityIdInEJBType());
...
function setFFC(){
    ...
    var shipmodeId = '<%= UIUtil.toJavaScript(shipIdr) %>';
...
<H1><%= UIUtil.toHTML((String)resources.get("PackageDetailPanel")) %></H1>   
    ...
    <TABLE>
       <TBODY>
		...
		<TR><TD>
			...
			<INPUT TYPE = TEXT VALUE = "" NAME=PDPACKAGEID ID="PDPACKAGEID" SIZE=20 maxlength=20/>
		</TD></TR>

		<TR><TD>
			<LABEL  for="PDTRACKINGID"><%= UIUtil.toHTML((String)resources.get("PDTrackingID")) %></LABEL>
			<INPUT TYPE = TEXT VALUE = "" NAME=PDTRACKINGID ID="PDTRACKINGID" SIZE=20 maxlength=20/>
		</TD></TR>

		<TR><TD>
			<LABEL  for="PDPICKUPID"><%= UIUtil.toHTML((String)resources.get("PDPickupID")) %></LABEL>    
			<INPUT TYPE = TEXT VALUE = "" NAME=PDPICKUPID ID="PDPICKUPID" SIZE=20 maxlength=20/>
		</TD></TR>

		<TR><TD>
		<TABLE>
    			<TR>
      				<TD><LABEL for="shipmodeId"><%= UIUtil.toHTML((String)resources.get("PDShippingProvider")) %></LABEL></TD>
    			</TR>
		...
		<TABLE>
         		...
         		<TR>
					...
					String unitDisplay = (String) vecDesc.elementAt(i);
      					...
      					<%= unitDisplay %></OPTION><%firstTime = 0;}%></SELECT>
				    </TD>
           			<TD>&nbsp;</TD>
         		...
         		<TR>
           			...
           			<TD></TD><TD ><SELECT NAME=Currency ID="Currency" ></SELECT></TD>
           			<TD>&nbsp;</TD>
          		</TR>
		</TABLE>
		</TD></TR>
