...
<%
	...
	Integer storeDefLangId 		= cmdContext.getStore().getLanguageIdInEJBType();
			...
			for (int i = 0; i < usagesTemp.size(); i++) {
				...
				if((usageTemp != null) && (ECAttachmentConstants.EC_ATCH_USAGE_DEFAULT_ID.equals(usageTemp.elementAt(0).toString()))){
					...
					while (enAttachmentRelationTemp.hasMoreElements()) {
						...
						tempList.add(abAttachmentRelationTemp.getAttachmentRelationIdInEJBType());
					}
				}
			}
		}
...
<%
				...
				while (enAttachmentRelation.hasMoreElements()) {
					...
					try {
						...
						abAttachmentRelationDescription = abAttachmentRelationDescription.findByAttachmentRelationIdAndLanguageId(abAttachmentRelation.getAttachmentRelationIdInEJBType(), storeDefLangId);
					...
					<table class=attachmentPreviewTable<%=(atchRelCount%2)+1%> cellpadding=0 cellspacing=0 border=0 style="width:98%">
						<tr>
							<td rowspan=99 align=center bgcolor=#D3D4DB width=25>
                               <input <%=disabledString%> id="<%=abAttachmentRelation.getAttachmentRelationIdInEJBType()%>" type=checkbox onclick="checkBoxEvent(event, <%=nousage%>)"/>
								<input id=targetId_<%=abAttachmentRelation.getAttachmentRelationIdInEJBType()%> type=hidden value="<%=atchTargetId%>">
							</td>
						</tr>
...
<% 					if (ATCHREL_DESCRIPTION_FOUND) 
					{ %>
							<tr><td colspan=3 align=left style="word-break:break-all;width:100%;"><font size=1><b>&nbsp;<label for="<%=abAttachmentRelation.getAttachmentRelationIdInEJBType()%>"><%=UIUtil.toHTML((String) abAttachmentRelationDescription.getName())%></label><br/></b></font></td></tr>
<%		}			else 
					{%>
							<tr><td colspan=3></br><label for="<%=abAttachmentRelation.getAttachmentRelationIdInEJBType()%>" class="hidden-label"><%=UIUtil.toHTML((String) rbAttachment.get("AttachmentList_Label_NoRelationshipName"))%></label></td></tr>
<%					}%>
					...
					while (enAttachmentAsset.hasMoreElements()) {
						...
						if (!SINGLELANGSTORE) {
							...
							while (enAttachmentAssetLanguage.hasMoreElements()) {
								...
								try {
									languageDB.setDataBeanKeyDescriptionLanguageId(abAttachmentAssetLanguage.getLanguageIdInEJBType().toString());
