...
public String getFormattedAmount(BigDecimal amount, String currency, Integer langId, String storeId) {
	try {
		...
		FormattedMonetaryAmountDataBean formattedAmount = new FormattedMonetaryAmountDataBean(new MonetaryAmount(amount, currency), iStoreDB, langId);
...
if (customerId == null || customerId.length() == 0) {
	...
	if (searchOrderId != null && searchOrderId.length() != 0) {
		try {
			...
			ECTrace.trace(ECTraceIdentifiers.COMPONENT_ORDER, "OrderListB2C.jsp", "none", "Exception when populating orderDB");
		}
	}
...
<script type="text/javascript"
	src="/wcs/javascript/tools/common/Util.js"></script>
<script type="text/javascript"
...
function checkForReturnableOrder() {
    ...
    //For standard order store, only "S","D","P" orders are allowed to return.
	  for (var i=0; i<checkeds.length; i++) {
		for (var j=0; j<list.length; j++) {
			if (list[j].orderId == checkeds[i]) {
	...
	checkeds = parent.getChecked();
	  ...
	  for (var i=0; i<checkeds.length; i++) {
		for (var j=0; j<list.length; j++) {
			if (list[j].orderId == checkeds[i]) {
				if (list[j].state == "X") {
					alertDialog("<%=UIUtil.toJavaScript((String) orderLabels.get("invalidSelectionForInvoice"))%>");
					return false;
				}
				if (list[j].state == "T") {
					alertDialog("<%=UIUtil.toJavaScript((String) orderLabels.get("invalidSelectionLocked"))%>");
...
// need to check order status type  & type ORD can block manag
	for (var j=0; j<list.length; j++) {
		if (list[j].orderId == orderId) {
			if ( list[j].status == "D" ||list[j].status == "X" || list[j].orderType != "ORD") {
				alertDialog("<%=UIUtil.toJavaScript((String) orderLabels.get("invalidSelectionForBlock"))%>");
				return false;
			}
		}
...
<%=comm.addDlistColumnHeading((String) orderLabels.get("orderNumber"), "orderId", orderByParam.equals("orderId"))%>
<%=comm.addDlistColumnHeading((String) orderLabels.get("customerName"), "logonId", orderByParam.equals("logonId"))%>
<%=comm.addDlistColumnHeading((String) orderLabels.get("blockStatus"), null, false)%>
<%=comm.addDlistColumnHeading((String) orderLabels.get("paymentStatus"), null, false)%>
<%=comm.addDlistColumnHeading((String) orderLabels.get("orderStatus"), null, false)%>
<%=comm.addDlistColumnHeading((String) orderLabels.get("orderDate"), "ordate", orderByParam.equals("ordate"))%>
<%=comm.addDlistColumnHeading((String) orderLabels.get("orderUpdatedDate"), "updateDate", orderByParam.equals("updateDate"))%>
<%=comm.addDlistColumnHeading((String) orderLabels.get("total"), "ortotal", orderByParam.equals("ortotal"))%>
<%=comm.endDlistRow()%>
...
<%=comm.addDlistColumn(UIUtil.toHTML(oList.getOrderListData(i).getMemberName()), "none")%>
<%
 //here need to use new databean or list data bean has been changed
...
<%aStrBuffer = new StringBuffer();
//get currency from Order so that it gets converted proprely
...
<%=comm.endDlistRow()%> <%if (rowselect == 1) {
	rowselect = 2;
} else {
	rowselect = 1;
}
...
<table cellspacing="0" cellpadding="3" border="0">
	<tr>
		<td colspan="7"><%=orderLabels.get("noOrdersToList")%></td>
	</tr>
</table>
<%}
%></form>
