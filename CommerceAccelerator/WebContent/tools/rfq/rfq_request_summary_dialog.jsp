...
<%
    ...
    if (rfq.getStartTimeInEJBType() !=null) {
		...
		rfq_start = df_date.format(rfq.getStartTimeInEJBType()) + " " + df_time.format(rfq.getStartTimeInEJBType());
    } else {
		rfq_start = "";
    }
    String rfq_end = null;
    if (rfq.getEndTimeInEJBType() !=null) {
		...
		rfq_end = df_enddate.format(rfq.getEndTimeInEJBType()) + " " + df_endtime.format(rfq.getEndTimeInEJBType());
    } else {
		rfq_end ="";
    }

    String rfq_close = null;
    if (rfq.getCloseTimeInEJBType() !=null) {
		...
		rfq_close = df_closedate.format(rfq.getCloseTimeInEJBType()) + " " + df_closetime.format(rfq.getCloseTimeInEJBType());
    } else {
		rfq_close ="";
    }
...
if (allProductsSelected.equals("1")) {
	...
	for (int i = 0; pList != null && i < pList.length; i++) {
		...
		BigDecimal price_ejb = aPList.getPriceInEJBType();
