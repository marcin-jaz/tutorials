 ...
 */
 	CKEDITOR.plugins.add('ibmtoolbars',
	{
		...
		init : function( editor )
		{
			...
			//only support floating toolbar if the ibmFloatToolbar config option is true. Do not support for IE6 or quirks mode (position:fixed does not work on these browsers). Do not support in inline mode - the floatingspace plugin provides similar functionality for inline editors.
			if (!editor.config.ibmFloatToolbar || (CKEDITOR.env.ie && CKEDITOR.env.version < 7 ) || (CKEDITOR.env.quirks && CKEDITOR.env.ie) || editor.elementMode == CKEDITOR.ELEMENT_MODE_INLINE)
				return;
			...
			var resetToolbarProps = function ()
			{
				if (toolbarFloated){
					...
					if (editor.config.width ){	
						...
						} else if (CKEDITOR.env.webkit || (CKEDITOR.env.ie && CKEDITOR.env.version == 7)){		//safari or ie7
							resetWidths(contentArea, true);
						} else {
...
[
	{ name: 'tools',		items :['Undo','Redo','MenuPaste','Find','IbmSpellChecker','ShowBlocks']},
