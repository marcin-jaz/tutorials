...
CKEDITOR.tools.extend(CKEDITOR.ibm.dialogs, {
	...
	cellProperties : function(dialogDefinition, editor) {
		...
		{
			var widthAttr = parseFloat( element.getAttribute( 'width' )),
				widthStyle = parseFloat( element.getStyle( 'width' ));
		...
		bgColorChooseButton.onLoad = function (){
			if(CKEDITOR.env.hc){
				...
				if (CKEDITOR.env.ie && CKEDITOR.env.version < 8) {
					this.getElement().getParent().setStyle( 'padding-bottom', '1px' );
					this.getElement().getParent().setStyle( 'padding-top', '5px' );
				} else {
		...
		var borderListColor = {
			...
			setup: function(element) {
				...
				var existingColor = (element.getStyle('border-color') || element.getAttribute('borderColor'));
		...
		var backgroundListColor = {
			...
			setup : function(element) {
				...
				//get the browser specific color value
				var existingColor = (element.getStyle('background-color') || element.getAttribute('bgColor'));
				...
				children: [
					heightField,
					{
						...
						setup: function(selectedCell) {
							...
							if ('' === selectedCell.$.style.height) {
									return;
							}

							var match = /^(\d+(?:\.\d+)?)px$/.exec(selectedCell.$.style.height);
							if (match) {
								this.setValue('px');
							}
						}
