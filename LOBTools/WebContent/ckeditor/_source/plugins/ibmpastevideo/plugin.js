	...
	init : function(editor)	{
		...
		function VideoParser(videoHtml) {
			...
			this.parse = function()  {
				...
				//ie6 won't set the innerHTML of our hidden DIV correctly unless the markup includes a classid. If classid does not already exist, add one
				var addedClassID = false;
				if (CKEDITOR.env.ie6Compat && _html.toLowerCase().indexOf('classid') == -1){
					var objTagIndex = _html.indexOf('object ')+7;	//7 = "object " length
					_html = _html.substring(0, objTagIndex) + "classid='clsid:d27cdb6e-ae6d-11cf-96b8-444553540000' " + _html.substring(objTagIndex);					
					addedClassID = true;
				}
				...
				//if we added the classid for ie6, remove it now - if we don't, the ckeditor source code generated for the video markup will not work in FF
				if(addedClassID){					
					objectNode = objectNode.replace(/classid=clsid:d27cdb6e-ae6d-11cf-96b8-444553540000/, "");
				}
