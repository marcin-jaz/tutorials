...
CKEDITOR.plugins.add('ibmmenuhelpmessage',
		{
			...
			init : function(editor) {
				...
				//Do not support for IE6
				if (editor.config.disableNativeSpellChecker || !editor.config.displayContextMenuHelpMessage 
								|| editor.config.browserContextMenuOnCtrl === false || (CKEDITOR.env.ie && CKEDITOR.env.version < 7))
							return;
					...
					for ( var i = 0; i < divElements.count(); i++) {
						...
						if(divElement){
							...
							if (CKEDITOR.env.ie && CKEDITOR.env.version < 8)
								messageDiv = createMenuItem("div", iframeDoc, 'cke_contextmenu_message_ie7');
				...
				function displayHelpMessage(divElement,style) {
					if (divElement.getAttribute('class') == "cke_panel_block" && (!style || style !='display: none;')) {
						...
						if (divElement.getAttribute('class') == "cke_panel_block" && (!style || style !='display: none;')) {
							...
							if (divEl.getAttribute('class') == "cke_menu") {
