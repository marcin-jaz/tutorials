...
<%
  ...
  //227736
  if (startTime!=null && (startTime.indexOf("<")!=-1 || startTime.indexOf(">")!=-1)){
    startTime = null;
  }
  if (endTime!=null && (endTime.indexOf("<")!=-1 || endTime.indexOf(">")!=-1)){
    endTime = null;
  }
   ...
   try {
      ...
      //Avoid displaying job status of other stores than the site.
      //schedulerItems.setStoreId(new Integer(0));
...
function getResultsSize() {
     ...
     function addJob() {
          var url = top.getWebappPath() + "DialogView?XMLFile=adminconsole.SchedulerAdd";

          if (top.setContent)
          {
               top.setContent("<%= UIUtil.toJavaScript((String)schedulerNLS.get("schedulerAddTitle")) %>",
                         url,
                         true);
          }
          else
     ...
     function addBroadcastJob() {
          var url = top.getWebappPath() + "DialogView?XMLFile=adminconsole.SchedulerAddBroadcast";

          if (top.setContent)
          {
               top.setContent("<%= UIUtil.toJavaScript((String)schedulerNLS.get("schedulerAddBroadcastTitle")) %>",
                         url,
                         true);
          }
          else
     ...
     function editJob() {
          ...
          var typeTest = "";
          if (checked[0].length > <%= ECConstants.EC_SCHED_BROADCAST.length() %>) typeTest = checked[0].substring(checked[0].length - <%= ECConstants.EC_SCHED_BROADCAST.length() %>, checked[0].length);
          var url;
          if ( typeTest == ('<%= ECConstants.EC_SCHED_BROADCAST %>') ) url = "DialogView?XMLFile=adminconsole.SchedulerAddBroadcast&" + checked[0];
          else url = top.getWebappPath() + "DialogView?XMLFile=adminconsole.SchedulerAdd&" + checked[0];
          if (top.setContent)
          {
               top.setContent("<%= UIUtil.toJavaScript((String)schedulerNLS.get("schedulerEditTitle")) %>",
                         url,
                         true);
          }
          else
     ...
     function editJobBySelect(item) {
          var typeTest = "";
          if (item.length > <%= ECConstants.EC_SCHED_BROADCAST.length() %>) typeTest = item.substring(item.length - <%= ECConstants.EC_SCHED_BROADCAST.length() %>, item.length);
          var url;
          if ( typeTest == ('<%= ECConstants.EC_SCHED_BROADCAST %>') ) url = "DialogView?XMLFile=adminconsole.SchedulerAddBroadcast&" + item;
          else url = top.getWebappPath() + "DialogView?XMLFile=adminconsole.SchedulerAdd&" + item;
          if (top.setContent)
          {
               top.setContent("<%= UIUtil.toJavaScript((String)schedulerNLS.get("schedulerEditTitle")) %>",
                         url,
     ...
     function removeJob() {
          ...
          if ( confirmDialog('<%= UIUtil.toJavaScript((String)schedulerNLS.get("schedulerRemoveConfirm")) %>') ) {
               ...
               var url = top.getWebappPath() + 'RemoveJob?URL=MsgMessagingSystemResponseView&' + checked[0];
...
%>
     ...
     function cleanJobStatus() {
          ...
          if ( confirmDialog('<%= UIUtil.toJavaScript((String)schedulerNLS.get("schedulerCleanConfirm")) %>') ) {
               var checked = parent.getChecked();
               var url = top.getWebappPath() + 'CleanJob?URL=MsgMessagingSystemResponseView&endTime=<%= rightNowStr %>&' + checked[0];
               top.setContent("", url, true);
          }
     }
     ...
     function cleanAllJobStatus() {
          ...
          if ( confirmDialog('<%= UIUtil.toJavaScript((String)schedulerNLS.get("schedulerCleanAllConfirm")) %>') ) {
               var url = top.getWebappPath() + 'CleanJob?URL=MsgMessagingSystemResponseView&endTime=<%= rightNowStr %>';
               top.setContent("", url, true);
          }
     }
     ...
     function changeCriteria() {
         ...
         var url = top.getWebappPath() + "DialogView?ActionXMLFile=adminconsole.SchedulerMain&amp;XMLFile=adminconsole.SchedulerCriteria&amp;cmd=SchedulerMainView&amp;listsize=<%=Integer.toString(listSize)%>&amp;startindex=<%=Integer.toString(startIndex)%>&amp;orderby=<%=UIUtil.toJavaScript(orderByParm)%>&amp;startDateCriteria=<%=UIUtil.toJavaScript(startTime)%>&amp;endDateCriteria=<%=UIUtil.toJavaScript(endTime)%>&amp;refnum=0'";
         parent.location.replace(url);
...
<FORM NAME="schedulerForm" ACTION="SchedulerMainView" METHOD="POST">
