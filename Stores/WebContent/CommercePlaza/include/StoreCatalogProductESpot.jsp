...
--%>
	...
	<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}" varStatus="counter">
		...
		<c:if test='${CategoriesCounter==1}'>
			<tr>
				<td class="tblue" height="20" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter }" />">&nbsp;<fmt:message key="StoreCatalogDisplay_Featured" bundle="${storeText}" /></td>
			</tr>
		...
		<tr>
				<td id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter + 2}" />">
...
<c:set var="numberOfResults" value="0" />
	...
	<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}" varStatus="status">
		<c:if test='${marketingSpotData.dataType eq "CatalogEntry"}'>
			...
			<c:if test='${status.count==1 && titleShown == "false"}'>
				<tr>
					<td class="tblue" height="20" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter }" />">&nbsp;<fmt:message key="StoreCatalogDisplay_Featured" bundle="${storeText}" /></td>
				</tr>
				<c:set var="titleShown" value="true" />
		                ...
		                <c:set var="imageFilePath" value="${staticAssetContextRoot}/${attribute.stringValue.value}/" />
							...
							<a href="<c:out value="${marketing_ClickInfoURL}"/>" id="<c:out value="${marketing_Link_ID_Prefix}" /><c:out value="${marketing_LinkCounter}" />">
								<img 
									...
									alt="<c:out value="${marketing_CatalogEntryName}" escapeXml="false"/>" width="90" height="90" hspace="5" border="0" />
							</a>
						</td>
					</tr>
					<tr>
						<td class="productName" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter + 2}" />">
