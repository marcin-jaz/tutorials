...
orderListBean.setSortMethodId(1); // sort by last update
  ...
  <tr>
    ...
    <td valign="top" width="590">
      ...
      <table width="590" border="0" cellspacing="0" cellpadding="0">
  	    ...
  	    <tr>
  	      <td width="590" height="1" colspan="3"><img src="<%=fileDir%>images/c.gif" width="2" height="1" /></td>
  	    </tr>
      </table>	    
      <form name="TimeRange" action="DistributorCartsByDateDisplayView" method="post">
              ...
              <td>
                <table cellpadding="1" cellspacing="1" border="0" width="590">
                  ...
                  <tr>
                    ...
                    <td colspan="4" align="right"><b><%=storeText.getString("DistributorCarts_SortBy")%> <a href="DistributorCartsByStatusDisplayView?storeId=<%=storeId%>&catalogId=<%=catalogId%>&langId=<%=languageId%>"><%=storeText.getString("DistributorCarts_Status")%> </a> | <a href="DistributorCartsByDistributorDisplayView?storeId=<%=storeId%>&catalogId=<%=catalogId%>&langId=<%=languageId%>"><%=storeText.getString("DistributorCarts_Distributor")%></a> | <%=storeText.getString("OrderList_LastUpdated")%> / <%=storeText.getString("DistributorCarts_Date")%></b></td>
                  </tr>                
                  <tr class="tdblue">
    		    <td colspan="7" height="18" class="tdblue">&nbsp;&nbsp;<%=storeText.getString("DistributorCarts_YourCarts")%></td>
      	          </tr>
                  <tr valign="top"> 
...
%>
                  <tr>
            	    ...
            	    <td headers="t2" height="19" align="left" valign="top"><span class="small"><%= distributorName %></span></td>
<%
  			String orderDesc = "";
...
%>
		    <td headers="t3" height="19" align="left" valign="top"><span class="small"><a href="PartialTransferShopCartDisplayView?orderId=<%=parentOrderBean.getOrderId()%>&catalogId=<%=catalogId%>&langId=<%=languageId%>"><%= orderDesc %></a></span></td>
<%  			} else {     %>			
		    <td headers="t3" height="19" align="left" valign="top"><span class="small"><%= orderDesc %></span></td>
<%			}  %>
		    <td headers="t4" align="left" nowrap valign="top"><span class="small"><%= statusValue %></span></td>
<%		  	if (orderBean.getStatus().equals("P") || orderBean.getStatus().equals("F") || orderBean.getStatus().equals("W") || orderBean.getStatus().equals("N")) {  %>
            	    <td headers="t5" height="19" align="left" nowrap valign="top"><span class="small"><%=storeText.getString("DistributorCarts_NotAvailable")%></span></td>
            	    <td headers="t6" height="19" align="left" nowrap valign="top"><span class="small"><%= TimestampHelper.getDateFromTimestamp(orderBean.getLastUpdateInEJBType(),locale) %></span></td>
<%	  		} else {         
          		  String extURL = null;
          		  try{
  			  ...
  			  if(( extURL!=null)&&(extURL.length()>0)&&(!extURL.equals("null"))) {  %>				
            	    <td headers="t5" height="19" align="left" nowrap valign="top"><a href="ExitToDistributorSiteDisplayView?storeId=<%=storeId%>&langId=<%=languageId%>&forwardURL=<%=java.net.URLEncoder.encode(extURL)%>" target="_blank"><span class="small"><%= orderStatusDB.getMerchantOrderNumber() %></span></a></td>
<%	      		  }	else	{ %>
            	    <td headers="t5" height="19" align="left" nowrap valign="top"><span class="small"><%= orderStatusDB.getMerchantOrderNumber() %></span></td>
<%			  } %>
            	    <td headers="t6" height="19" align="left" nowrap valign="top"><span class="small"><%= TimestampHelper.getDateFromTimestamp(orderBean.getPlaceOrderTimeInEJBType(),locale) %></span></td>
<%		  	} //end if
%>
