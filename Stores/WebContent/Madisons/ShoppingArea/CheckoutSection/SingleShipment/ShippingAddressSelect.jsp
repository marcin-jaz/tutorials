...
<div class="shipping_address" id="WC_ShippingAddressSelectSingle_div_1">
	<p class="title"><label for="singleShipmentAddress"><fmt:message key="SHIP_SHIPPING_ADDRESS" bundle="${storeText}"/></label>:</p>
	<p>
	   <select class="drop_down" name="singleShipmentAddress" id="singleShipmentAddress" onChange="JavaScript:setCurrentId('singleShipmentAddress'); CheckoutHelperJS.displayAddressDetails(this.value,'Shipping');CheckoutHelperJS.updateAddressForAllItems(this);CheckoutHelperJS.showHideEditAddressLink(this,'<c:out value='${param.orderId}'/>')">
			<c:forEach var="contactInfoIdentifier" items="${orderShipInfo.usableShippingAddress}">
				...
				>
					...
					<c:choose>
						<c:when test="${contactInfoNickName eq  profileShippingNickname}"><fmt:message key="QC_DEFAULT_SHIPPING" bundle="${storeText}"/></c:when>
						<c:when test="${contactInfoNickName eq  profileBillingNickname}"><fmt:message key="QC_DEFAULT_BILLING" bundle="${storeText}"/></c:when>
	...
	<flow:ifEnabled feature="AjaxCheckout"> 
		<div dojoType="wc.widget.RefreshArea" id="shippingAddressDisplayArea" widgetId="shippingAddressDisplayArea" controllerId="shippingAdddressDisplayAreaController" role="wairole:region" waistate:live="polite" waistate:atomic="true" waistate:relevant="text">
...
<c:if test="${orderBean.personalAddressesAllowedForShipping}">
	...
	<c:if test="${selectedAddressId != -1}" >
		<br/>
		<div id="editAddressLink_<c:out value='${param.orderId}'/>" style="display:block;">
			<a href="JavaScript:CheckoutHelperJS.editAddress('singleShipmentAddress','-1','<c:out value='${fn:replace(profileshipping,search01,replaceStr01)}'/>','<c:out value='${fn:replace(profilebilling,search01,replaceStr01)}'/>')" id="WC_ShippingAddressSelectSingle_link_1">
				<img src="<c:out value='${jspStoreImgDir}'/>images/edit_icon.png" alt="<fmt:message key="ADDR_EDIT_ADDRESS" bundle="${storeText}"/>" />
				<fmt:message key="ADDR_EDIT_ADDRESS" bundle="${storeText}"/><span class="spanacce"><fmt:message key="Checkout_ACCE_Edit_Shipping" bundle="${storeText}"/></span>
			</a>
		</div>
	...
	<!-- new address link -->
	<div id="newShippingAddressLink">
			<a href="JavaScript:setCurrentId('singleShipmentAddress'); CheckoutHelperJS.addNewShippingAddress('Shipping');JavaScript:setCurrentId('singleShipmentAddress'); CheckoutHelperJS.updateAddressForAllItems('singleShipmentAddress');">
...
<script type="text/javascript">
			dojo.addOnLoad(function() { 
				CheckoutHelperJS.showHideEditAddressLink(document.getElementById("singleShipmentAddress"),'<c:out value='${param.orderId}'/>');
			});
</script>
</div>
