<%--
 ...
 =================================================================
--%><%--
  * This ESpotNavDisplay.jsp file is built for displaying an e-Marketing Spot in the sidebar of the
  ...
  *		<c:import url="${jspStoreDir}include/ESpotNavDisplay.jsp">
...
%>   
  ...
  --%>
	...
	<c:set var="WC_RightSidebarESpotDisplay_div_1_ID" value="WC_RightSidebarESpotDisplay_div_1_${emsName}"/>
	<c:set var="WC_RightSidebarESpotDisplay_div_2_ID" value="WC_RightSidebarESpotDisplay_div_2_${emsName}"/>	
  ...
  --%>
<div class="genericESpot" id="${fn:replace(WC_RightSidebarESpotDisplay_div_1_ID, search, replaceStr)}"><div class="caption" style="display:none" id="${fn:replace(WC_RightSidebarESpotDisplay_div_2_ID, search, replaceStr)}">[<c:out value="${emsName}"/>]</div>
          ...
          <c:forEach var="attribute" items="${marketingSpotData.catalogGroup.attributes}">	
             <c:if test='${attribute.key eq "rootDirectory"}'>				    
                <c:set var="imageFilePath" value="${staticAssetContextRoot}/${attribute.value}/" />
					...
					<div class="<c:out value="${param.adclass}"/>" id="<c:out value='${param.adclass}_${currentCategoryCount}'/>">
						<p>
						<c:choose>				   
							<c:when test="${!empty marketing_catalogGroupImage}">
								<a id="WC_RightSidebarESpotDisplay_links_1_<c:out value='${status.count}'/>" href="<c:out value="${absoluteUrl}${ClickInfoURL}" escapeXml="false"/>">
									<img class="rightCategoryRecommendation" src="<c:out value="${hostPath}${imageFilePath}${marketing_catalogGroupImage}"/>" alt="<c:out value="${marketing_catalogGroupShortDescription}" />" border="0" />
								</a>
							</c:when>
							<c:otherwise>
								<a id="WC_RightSidebarESpotDisplay_links_2_<c:out value='${status.count}'/>" href="<c:out value="${absoluteUrl}${ClickInfoURL}" escapeXml="false"/>">
...
--%>	
	...
	<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}" varStatus="status2">
		<c:if test='${marketingSpotData.dataType eq "CatalogEntry"}'>
			<c:if test="${!headerDisplayed}">
				<div id="recommend">
					<div class="header" id="WC_RightSidebarESpotDisplay_div_3_<c:out value='${status2.count}'/>">
						 <h2 class="sidebar_header"><fmt:message key="RECOMMENDATIONS" bundle="${storeText}" /></h2>
						 <p><fmt:message key="YOU_MAY_LIKE" bundle="${storeText}" /></p>
					</div>
					<div class="content" id="WC_RightSidebarESpotDisplay_div_4_<c:out value='${status2.count}'/>">
          ...
          <c:forEach var="attribute" items="${marketingSpotData.catalogEntry.catalogEntryAttributes.attributes}">	
			...
			<div class="itemWrapper">
					...
					<c:set var="pageView" value="sidebar"/>
	...
	<c:if test="${currentProductCount != 0}">
					</div>
					<div class="footer" id="WC_RightSidebarESpotDisplay_div_5"><img src="${jspStoreImgDir}${vfileColor}recommend_bot.png" alt="" /></div>
				</div>
	</c:if>
<%--
  *
...
--%>
	...
	<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}" varStatus="status3">
		<c:if test='${marketingSpotData.dataType eq "MarketingContent"}'>
				...
				<c:choose>
					<c:when test="${!empty param.adclass}">
						<div class="<c:out value="${param.adclass}"/>" id="<c:out value='${param.adclass}_${currentContentCount}'/>">
					</c:when>
					<c:otherwise>
						<p>
					</c:otherwise>
        ...
        --%>	
			  ...
			  <c:choose>
	              ...
	              <c:when test="${(marketingSpotData.marketingContent.mimeType eq 'image') || (marketingSpotData.marketingContent.mimeType eq 'images')}">		
                   ...
                   <c:if test="${!empty marketingSpotData.marketingContent.url}">
										  <a id="WC_RightSidebarESpotDisplay_links_3_<c:out value='${status3.count}'/>" href="${absoluteUrl}${ClickInfoURL}" ${clickOpenBrowser} >
										</c:if>
											<img
                   ...
                   --%>		
	                    <c:choose>
                         <c:when test="${(attachment.attachmentAsset[assetIndex].mimeType eq 'application/x-shockwave-flash')}" >
                            <object data='<c:out value="${hostPath}"/>${staticAssetContextRoot}/${attachment.attachmentAsset[assetIndex].rootDirectory}/<c:out value="${attachment.attachmentAsset[assetIndex].attachmentAssetPath}"/>'
							...
							<param name="movie" value="<c:out value="${hostPath}"/>${staticAssetContextRoot}/${attachment.attachmentAsset[assetIndex].rootDirectory}/<c:out value="${attachment.attachmentAsset[assetIndex].attachmentAssetPath}"/>"/>
                         ...
                         <c:otherwise>
                            <a id="WC_RightSidebarESpotDisplay_links_4_${status3.count}" href="<c:out value="${hostPath}"/>${staticAssetContextRoot}/${attachment.attachmentAsset[assetIndex].rootDirectory}/<c:out value="${attachment.attachmentAsset[assetIndex].attachmentAssetPath}"/>" target="_blank"> 
                            <img
                               src='<c:out value="${hostPath}"/>${staticAssetContextRoot}/${attachment.attachmentAsset[assetIndex].rootDirectory}/<c:out value="${attachment.attachmentUsage.image}"/>'
                               alt='<c:out value="${attachment.attachmentDescription[descriptionIndex].shortDescription}"/>'
                               border="0"
                            />
                            </a>
                      ...
                      --%>	                      
 				              <c:if test="${!empty marketingSpotData.marketingContent.url}">
				                 <a id="WC_RightSidebarESpotDisplay_links_5_<c:out value='${status3.count}'/>" href="${absoluteUrl}${ClickInfoURL}" ${clickOpenBrowser} >
				              </c:if>
          ...
          <c:when test="${marketingSpotData.marketingContent.marketingContentFormat.name == 'Text'}">		  
             ...
             --%>			          	
 				     <c:if test="${!empty marketingSpotData.marketingContent.url}">
				       <a id="WC_RightSidebarESpotDisplay_links_7_<c:out value='${status3.count}'/>" href="${absoluteUrl}${ClickInfoURL}" ${clickOpenBrowser} >
				     </c:if>
					   <c:out value="${marketingSpotData.marketingContent.marketingContentDescription[0].marketingText}" escapeXml="false" />
             <c:if test="${!empty marketingSpotData.marketingContent.url}">	           
					     </a>
