...
<c:choose>
	<c:when test="${WCParam.URL != null && WCParam.URL!=''}" >
		<input type="hidden" name="URL" value="<c:out value="${requestScope.requestURIPath}" />?URL=<c:out value="${WCParam.URL}"/>" id="WC_SidebarDisplay_FormInput_URL_1"/>
	</c:when>
	<c:otherwise>
		<input type="hidden" name="URL" value="<c:out value="${requestScope.requestURIPath}" />" id="WC_SidebarDisplay_FormInput_URL_1"/>
	</c:otherwise>
</c:choose>
