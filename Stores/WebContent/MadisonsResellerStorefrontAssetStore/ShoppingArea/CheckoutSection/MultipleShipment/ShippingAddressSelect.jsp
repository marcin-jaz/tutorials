...
<div class="shipping_address_MS_shipping_info_page" id="WC_ShippingAddressSelectMultiple_div_1">
<div id="MS_ShippingAddress_Section_<c:out value='${orderItemId}'/>" class="${requestScope.shippingModeVisibility}">
	<label for="singleShipmentAddress"></label>
	<p>
		<c:choose>
			<c:when test="${hasValidAddresses}" >
			<label for="MS_ShipmentAddress_<c:out value='${orderItemId}'/>"></label>
				<select class="drop_down form_input" name="MS_ShipmentAddress_<c:out value='${orderItemId}'/>" id="MS_ShipmentAddress_<c:out value='${orderItemId}'/>" onChange="JavaScript:setCurrentId('MS_ShipmentAddress_<c:out value='${orderItemId}'/>'); CheckoutHelperJS.updateAddressIdForThisItem(this.value,'<c:out value='${orderItemId}'/>');CheckoutHelperJS.displayAddressDetailsForMS(this.value,'<c:out value='${orderItemId}'/>','Shipping');CheckoutHelperJS.showHideEditAddressLink(this,'<c:out value='${orderItemId}'/>')">
					...
					><c:choose><c:when test="${contactInfoIdentifier.externalIdentifier.contactInfoNickName eq  profileShippingNickname}"><fmt:message key="QC_DEFAULT_SHIPPING" bundle="${storeText}"/></c:when>
					<c:when test="${contactInfoIdentifier.externalIdentifier.contactInfoNickName eq  profileBillingNickname}"><fmt:message key="QC_DEFAULT_BILLING" bundle="${storeText}"/></c:when>
						...
						<span class="button_bg">
							<span class="button_top">
								<span class="button_bottom">   
									<a href="JavaScript:CheckoutHelperJS.createAddress(-1,'Shipping');" id="WC_ShippingAddressSelectMultiple_link_1"><fmt:message key="ADD_ADDRESS" bundle="${storeText}"/></a>
								</span>
							</span>	
						</span>
					</span>
...
<c:catch>
	...
	<c:if test="${isPersonAddress}">
		...
		<c:if test="${selectedAddressId != -1}" >
			...
			<div id="editAddressLink_<c:out value='${orderItemId}'/>" style="display:block;">
				<c:if test="${!empty param.personalAddressAllowed && param.personalAddressAllowed}">
				<c:if test="${showEditShippingAddressLink}">
					<a href="JavaScript:CheckoutHelperJS.editAddress('MS_ShipmentAddress_<c:out value='${orderItemId}'/>','<c:out value='${orderItemId}'/>','<c:out value='${fn:replace(profileshipping,search01,replaceStr01)}'/>','<c:out value='${fn:replace(profilebilling,search01,replaceStr01)}'/>')" id="WC_ShippingAddressSelectMultiple_link_2_<c:out value='${orderItemId}'/>">
						...
						<span class="spanacce"><fmt:message key="Checkout_ACCE_Edit_Shipping" bundle="${storeText}"/></span>
					</a>
				 </c:if>
				 </c:if>
			</div>
	...
	<%-- Use the value of personalAddressAllowed to hide/show the create/edit address link. --%>
<c:if test="${!empty param.personalAddressAllowed && param.personalAddressAllowed}">
	<div id="newShippingAddressButton">
			<a href="JavaScript:setCurrentId('MS_ShipmentAddress_<c:out value='${orderItemId}'/>'); CheckoutHelperJS.addNewShippingAddressForMS('<c:out value='${orderItemId}'/>');CheckoutHelperJS.displayAddressDetailsForMSHelper('<c:out value='${orderItemId}'/>','Shipping');">
				<img class="nopadding" src="<c:out value='${jspStoreImgDir}${vfileColor}'/>table_plus_add.png" alt="<fmt:message key="ADDR_CREATE_ADDRESS" bundle="${storeText}"/>" />
...
<script type="text/javascript">
			dojo.addOnLoad(function() { 
				CheckoutHelperJS.showHideEditAddressLink(document.getElementById("MS_ShipmentAddress_<c:out value='${orderItemId}'/>"),'<c:out value='${orderItemId}'/>');
			});
</script>
</div>
</div>
