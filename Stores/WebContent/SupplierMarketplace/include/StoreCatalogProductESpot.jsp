...
--%>
	...
	<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}" varStatus="counter">
		...
		<c:if test='${CategoriesCounter==1}'>
			<b><fmt:message key="Home_ImageText" bundle="${storeText}" /></b><br>
		</c:if>
		...
		<c:choose>
			<c:when test="${!empty marketing_catalogGroupThumbNail}">
				...
				<img src="<c:out value="${hostPath}"/><c:out value="${imageFilePath}${marketing_catalogGroupThumbNail}"/>" alt="<c:out value="${marketing_CategoryName}" escapeXml="false"/>" width="150" height="150" border="0">
			</c:when>
			<c:otherwise>
				<img 
		...
		<font class="text"><c:out value="${marketing_CategoryName}" escapeXml="false"/>&nbsp;</font><br><br>
...
<c:set var="numberOfResults" value="0" />
	...
	<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}" varStatus="status">
		<c:if test='${marketingSpotData.dataType eq "CatalogEntry"}'>
			...
			<c:if test='${status.count==1 && CategoresCounter==0}'>
				<b><fmt:message key="Home_ImageText" bundle="${storeText}" /></b><br>
			</c:if>
			...
			<c:choose>
				<c:when test="${!empty marketing_catalogGroupThumbNail}">
					...
					<img src="<c:out value="${hostPath}"/><c:out value="${imageFilePath}"/><c:out value="${marketing_CatalogEntryThumbNail}"/>" alt="<c:out value="${marketing_CatalogEntryName}" escapeXml="false"/>" width="150" height="150" border="0">
				</c:when>
				<c:otherwise>
					<img 
						src="<c:out value="${hostPath}"/><c:out value="${jspStoreImgDir}" />images/NoImageIcon_sm45.jpg"
			...
			<font class="text"><c:out value="${marketing_CatalogEntryName}" escapeXml="false"/>&nbsp;</font><br><br>
