...
//*--------------------------------------------------------------------------------------
		...
		<c:if test="${! empty ccnumber  &&  empty param.paymentPolicyString}">
			<c:set var="cc_number" value="${ccnumber}" scope="request"/>
			<c:set var="cc_year" value="${ccyear}" scope="request"/>
			<c:set var="cc_month" value="${ccmonth}" scope="request"/>
			
		</c:if>
		<c:if test="${!(! empty ccnumber  && empty param.paymentPolicyString)}">
			<c:set var="cc_number" value="${param.cardNumber}" scope="request"/>
			<c:set var="cc_year" value="${param.cardExpiryYear}" scope="request"/>
			<c:set var="cc_month" value="${param.cardExpiryMonth}" scope="request"/>
		</c:if>
	
	

	//----------------------------------------------------------
	// BidForm's submit action will call this function
	//----------------------------------------------------------
	function policyCheck() {
			var form1 = self.document.BidForm;
		
			var policyChange = form1.PolicyChange.selectedIndex;
			if(policyChange <= 0){
...
//alert("1");
		self.document.BidForm.paymentPolicyString.value = BuildPolicyString();
		return true;
//alert("2==> " + self.document.BidForm.paymentPolicyString.value );
		//policyInit();
//alert(fetchBidFormNvps());
	}
