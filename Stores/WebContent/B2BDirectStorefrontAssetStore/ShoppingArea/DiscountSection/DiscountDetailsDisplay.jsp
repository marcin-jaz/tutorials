																	...
																	<td valign="top" width="100" id="WC_DiscountDetailsDisplay_TableCell_11_<c:out value="${counter.count}"/>">
																		...
																		<c:choose>
																			...
																			<%-- show the list price only if it is greater than the discountCatalogEntryDB.productDataBean price and if the discountCatalogEntryDB.productDataBean does not have price range (i.e. min price == max price) --%>
																					...
																					<c:when test="${ listPriced && (!empty discountCatalogEntryDB.productDataBean.maximumItemPrice) && (discountCatalogEntryDB.productDataBean.maximumItemPrice.amount < displayPrice.amount) && (discountCatalogEntryDB.productDataBean.maximumItemPrice.amount == discountCatalogEntryDB.productDataBean.minimumItemPrice.amount)}" >
																			...
																			<c:otherwise>
																				...
																				<c:choose>
																					...
																					<c:when test="${ !listPriced &&(empty discountItemDB.calculatedContractPrice)}" >
																						<c:set var="productDataBeanPriceString"><fmt:message key="NO_PRICE_AVAILABLE" bundle="${storeText}" /></c:set>
																					</c:when>

																					<c:when test="${ listPriced && (!empty displayPrice) && (!empty discountItemDB.calculatedContractPrice) && (discountItemDB.calculatedContractPrice.amount < discountItemDB.listPrice.amount)}" >
																						...
																						<c:out value="${displayPrice}" escapeXml="false" /></span>
																						<br />
