...
<%--
  * This MiniProductESpot.jsp file is built as a sample snippet to display an e-Marketing Spot in a
...
<%-- START - the following code can be used to drop multiple e-Marketing Spots onto the page.
     Customize the appropriate EMarketingSpot instance name and the e-Marketing Spot name before
     use. Duplicate this code if more than 1 spot is needed, but do not use the same spot name.
--%>

<%-- create ID/Counter for testing --%>


<c:set value="WC_eMarketingSpot_Table_" var="marketing_Table_ID_Prefix" />
<c:set value="WC_eMarketingSpot_Cell_" var="marketing_TableCell_ID_Prefix" />
<c:set value="WC_eMarketingSpot_Link_" var="marketing_Link_ID_Prefix" />
<c:set value="WC_eMarketingSpot_Flash_" var="marketing_Flash_ID_Prefix" />
	...
	<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}">
		<c:if test='${marketingSpotData.dataType eq "CatalogGroup"}'>
			<c:forEach var="description" items="${marketingSpotData.catalogGroup.description}" begin="0" end="0">
				...
				<c:set value="${description.longDescription}" var="marketing_CategoryLongDescription" />
			</c:forEach>
			...
			<tr valign="top">
				<td class="portlet_content" colspan="2" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter }" />">
					<strong><fmt:message key="eMarketingSpot_FeaturedCategory" bundle="${storeText}" /></strong>
				</td>
			</tr>
			<tr>
				<td class="portlet_content" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter}" />">
					<a class="portlet_content" href='<c:out value="${ClickInfoURL}"/>' id='<c:out value="${marketing_Link_ID_Prefix}" /><c:out value="${marketing_LinkCounter}" />'>
						<c:out value="${marketing_CategoryName}" escapeXml="false"/>
					</a>
				</td>
				<td class="portlet_content" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter + 1}" />">
					<c:out value="${marketing_CategoryLongDescription}" escapeXml="false"/>
				</td>
			</tr>
			<%-- Adjust counter  --%>
	...
	<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}">
		<c:if test='${marketingSpotData.dataType eq "CatalogEntry"}'>
			<c:forEach var="description" items="${marketingSpotData.catalogEntry.description}" begin="0" end="0">
				...
				<c:set value="${description.shortDescription}" var="marketing_CatalogEntryShortDescription" />
				<c:set value="${description.longDescription}" var="marketing_CatalogEntryLongDescription" />
			</c:forEach>
			...
			<tr valign="top">
				<td class="portlet_content" colspan="2" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter }" />">
					<strong><fmt:message key="eMarketingSpot_FeaturedProduct" bundle="${storeText}" /></strong>
				</td>
			</tr>
			<tr valign="top">
				<td class="portlet_content" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter + 1}" />">
             ...
             <c:if test='${attribute.name eq "rootDirectory"}'>				    
                <c:set var="imageFilePath" value="${staticAssetContextRoot}/${attribute.stringValue.value}/" />
					...
					<a href='<c:out value="${ClickInfoURL}"/>' id='<c:out value="${marketing_Link_ID_Prefix}" /><c:out value="${marketing_LinkCounter}" />'>
						<img src='<c:out value="${imageFilePath}"/><c:out value="${marketing_CatalogEntryThumbNail}"/>'
							alt='<c:out value="${marketing_CatalogEntryShortDescription}" />' border="0" />
					</a>
				</td>
				<td class="portlet_content" id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter + 2}" />">
					<a class="portlet_content" href='<c:out value="${ClickInfoURL}"/>' id='<c:out value="${marketing_Link_ID_Prefix}" /><c:out value="${marketing_LinkCounter + 1}" />'>
						<c:out value="${marketing_CatalogEntryShortDescription}" escapeXml="false"/>
					</a>
					<br/>
					<c:out value="${marketing_CatalogEntryLongDescription}" escapeXml="false"/>
				</td>
				<td class="portlet_price" id="<c:out value="${marketing_TableCell_ID_Prefix}${marketing_TableCellCounter + 3}" />">
					<br/>
					<c:set var="catalogEntryIdForPriceRule" value="${marketingSpotData.catalogEntry.catalogEntryIdentifier.uniqueID}"/>
					<%@ include file="../Snippets/ReusableObjects/GetCatalogEntryDisplayPrice.jspf"%>
					<c:out value="${displayPrice.amount}" escapeXml="false" />
					<c:out value="${displayPrice.currency}"/>
				</td>
			</tr>
				...
				<td id="<c:out value="${marketing_TableCell_ID_Prefix}" /><c:out value="${marketing_TableCellCounter + 1}" />">
                   ...
                   --%>			
									...
									<c:forEach var="attachmentAsset" items="${marketingSpotData.marketingContent.attachment.attachmentAsset}" begin="0" end="0">			        
										<img
										   src='<c:out value="${staticAssetContextRoot}/${attachmentAsset.rootDirectory}/${attachmentAsset.attachmentAssetPath}"/>'
