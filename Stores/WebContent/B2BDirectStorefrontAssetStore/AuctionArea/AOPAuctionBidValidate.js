...
function CheckResubmitWithPayment() {
	var policyChange = self.document.BidForm.PolicyChange.selectedIndex;
	if(policyChange <= 0){
	...
	self.document.BidForm.paymentPolicyString.value = pString;

	if ( FormSubmitted == 1 ) {
		var msg = "<fmt:message key="pleaseWaitMsg" bundle="${storeText}" />";
		alert(msg);
		return false;
	}
	...
	for (j=0; j<form1.elements.length; j++) {	
		...
		if (elem.name == "bidval" ) {         
			...
			if( isValid == false  || strippedBdPrice <= 0 || isInputStringEmpty( (elem.name).toString()) ) {
				var msg="<fmt:message key="bidValueMsg1" bundle="${storeText}" />";
				alert(msg);
				form1.bidval.focus()
				return false;
			} else {
				if( checkDigitsNum( strippedBdPrice ) == false ) {
					var msg="<fmt:message key="bidValueMsg2" bundle="${storeText}" />";
					alert(msg);
					form1.bidval.focus()
					return false;
				} else {
			...
			if( isValid == false || strippedInteger <= 0 || isInputStringEmpty( (elem.name).toString())) {
				var msg="<fmt:message key="quantityMsg1" bundle="${storeText}" />";
				alert(msg);
				...
				if( strippedInteger > form1.inventory_js.value ) {
					var msg="<fmt:message key="quantityMsg3" bundle="${storeText}" />";							      
					alert(msg);
					form1.bidquant.focus()
					return false;
				} else {
			...
			if( isValid == false || strippedMaxPrice <= 0 || isInputStringEmpty( (elem.name).toString()) ) {
				var msg="<fmt:message key="autobidLimit1" bundle="${storeText}" />";
				alert(msg);
				form1.maxbidlimit.focus()
				return false;
			} else {
				if( checkDigitsNum( strippedMaxPrice ) == false ) {
					var msg="<fmt:message key="autobidLimit2" bundle="${storeText}" />";							      
					alert(msg);
					form1.maxbidlimit.focus()
					return false;
				} else if( bidvalueexist == true && strippedMaxPrice <= strippedBdPrice ) {
					var msg="<fmt:message key="autobidLimit3" bundle="${storeText}" />";							      
					alert(msg);
					form1.maxbidlimit.focus()
					return false;  
				} else {
...
function ClearBidForm(form) {	
	
	for (i=0; i<form.elements.length; i++) {  
		elem = form.elements[i];                       
		
		if ( (elem.name == "bidval" )&& (elem.type=="text") 
			|| (elem.name == "bidquant") &&(elem.type=="text")
