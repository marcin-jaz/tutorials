				...
				<c:when test="${ContentObj.typeName == 'File'}">
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<td><a href="<c:out value="${ClickInfoFullPathVar}" escapeXml="false"/>"

								name="WC_ContentSpotDisplay_Link_9_<c:out value="${status.count}" />"
								id="WC_ContentSpotDisplay_Link_9_<c:out value="${status.count}" />">
							<c:out value="${ContentObj.marketingText}" escapeXml="false" />
							</a></td>
						</c:otherwise>
					</c:choose>
