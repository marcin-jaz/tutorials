...
<c:choose>
	<c:when test="${WCParam.URL != null && WCParam.URL!=''}" >
		<input type="hidden" name="URL" value="<c:out value="${requestScope.requestURIPath}" />?URL=<c:out value="${WCParam.URL}"/>" id="WC_SidebarDisplay_FormInput_URL_1_<c:out value='${param.count}'/>"/>
	</c:when>
	<c:otherwise>
		<input type="hidden" name="URL" value="<c:out value="${requestScope.requestURIPath}" />" id="WC_SidebarDisplay_FormInput_URL_1_<c:out value='${param.count}'/>"/>
	</c:otherwise>
</c:choose>
...
<c:forEach var="aParam" items="${WCParamValues}" varStatus="paramStatus">
	<c:forEach var="aValue" items="${aParam.value}"  varStatus="paramNumStatus">
		<c:if test="${aParam.key !='langId' && aParam.key !='logonPassword' && aParam.key !='URL' && aParam.key !='currency'}">
			<input type="hidden" name="<c:out value="${aParam.key}" />" value="<c:out value="${aValue}" />" id="WC_SidebarDisplay_FormInput_parm_<c:out value='${param.count}'/>_<c:out value="${paramStatus.count}" />_<c:out value="${paramNumStatus.count}" />"/>
		</c:if>
	</c:forEach>
</c:forEach>
