...
dojo.declare('wc.widget.WCRibbon', [dijit._Widget, dijit._Templated], {
        ...
        if( self.originalNode ) {
            ...
            dojo.query('.ibm-columns', this.originalNode).forEach( function(column, index) {
                ...
                dojo.query('> *', column).forEach( function(item, j) {
                    if (dojo.isChrome) {
                        // Chrome 29 and Dojo 1.5 workaround to ignore text nodes returned by dojo.query(">",)
                        if (item.nodeType != 3) {
	...
	checkHome: function(){		
		if(dojo.attr(this.ribbonContainer.parentNode,'id') == 'ibm-leadspace-head' && dojo.query('.ibm-home-page').length > 0){ return 1;}
		return 0;
	},
	...
	next: function() {		
		...
		if((this.currentSlideIndex + 1) == this.slideCount()){
			if(!this.checkHome()){this.duration = 1; } // to bring back to the first slide			
			...
			if(!this.checkHome()){this.currentSlideIndex=-1; }
			
		} else {
			// no, we can just go to right, as usual
			this.slideRight();
			if(!this.checkHome()){this.duration = this._durationCopy; // duration is reset to default duration
			}
...
dojo.declare('wc.widget.WCRibbonItem', [dijit._Widget, dijit._Templated, dijit._Container], {
    ...
    postCreate: function() {
    	...
    	case 1: 
    		this.attr('class', 'ibm-col-1-1'); break;
    	case 3:  
    		this.attr('class', 'ibm-col-6-2'); break;
    	case 5: 
    		this.attr('class', 'ibm-col-5-1'); break;
	    case 6:  
	    	this.attr('class', 'ibm-col-6-1'); break;
	    default: 
	    	this.attr('class', 'ibm-col-6-2'); break;
    	}
    }
		...
		var _temp = new wc.widget.WCRibbon({
			...
			customSeekAnimation: function(scrollWidth, slideToIndex, isCloned){
				...
				switch(this.currentSlideIndex) {
					case 1: var headlineNode = dojo.query('#ibm-lead-2 div.ibm-col-1-1')[0]; break;
					case 2: var headlineNode = dojo.query('#ibm-lead-3 div.ibm-col-1-1')[0]; break;
					default: var headlineNode = dojo.query('#ibm-lead-1 div.ibm-col-1-1')[0];
				}
