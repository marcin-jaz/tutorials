				...
				<div class="itemspecs" id="WC_OrderItemDetails_div_2_<c:out value='${status.count}'/>">
					...
					<c:forEach var="catalogEntry1" items="${catalogEntriesForAttributes}" >
						<c:if test="${catalogEntry1.catalogEntryIdentifier.uniqueID == orderItem.catalogEntryIdentifier.uniqueID}">
							...
							<a class="order_link" id="WC_OrderItemDetails_links_1_<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>" href="javascript:;"  onClick="JavaScript:var actionListAttributeSS = new popupActionProperties(); actionListAttributeSS.showAddToCart=false; actionListAttributeSS.showWishList=false; actionListAttributeSS.showProductCompare=false; actionListAttributeSS.showReplaceCartItem=true; showPopup('<c:out value='${parentCatEntry}'/>','<c:out value='${WCParam.storeId}'/>','<c:out value='${WCParam.langId}'/>','<c:out value='${WCParam.catalogId}'/>',event,null,'',actionListAttributeSS,${quickCartOrderItemQuantity}); saveChangeOrderItemId('${itemUniqueId}');"> 
