...
<c:if test="${empty orderInCart || orderInCart==null}">
	...
	<c:if test="${empty orderInCart || orderInCart==null}">
		...
		<wcf:getData type="com.ibm.commerce.order.facade.datatypes.OrderType"
					var="orderInCart" expressionBuilder="findCurrentShoppingCartWithPagingOnItem" varShowVerb="ShowVerbCart" maxItems="${maxOrderItemsToInspect}" recordSetStartNumber="0" recordSetReferenceId="headerorder">
			<wcf:param name="accessProfile" value="IBM_Details" />	 
			<wcf:param name="sortOrderItemBy" value="orderItemID" />
			<wcf:param name="isSummary" value="true" />
		</wcf:getData>
...
<div id="page">
       ...
       <!-- Breadcrumb Start -->
					...
					<c:choose>
						<c:when test="${empty validAddressId}">
							<c:if test="${(!b2bStore && orderItemsHaveInvalidAddressId) || (b2bStore && WCParam.guestChkout == '1' )}">
						 	...
						 	<span <c:if test="${b2bStore && !(WCParam.guestChkout == '1')}">class="on" </c:if> ><fmt:message key="BCT_SHIPPING_AND_BILLING" bundle="${storeText}"/></span> |
						</c:when>
						<c:otherwise>
							<span class="on"><fmt:message key="BCT_SHIPPING_AND_BILLING" bundle="${storeText}"/></span> |
						</c:otherwise>					
					</c:choose>
		...
		<div id="checkout_crumb">
              <div class="crumb" id="WC_ShipmentDisplay_div_4">
		<c:if test = "${!empty order.orderItem}">
                     <a href="<c:out value="${ShoppingCartURL}"/>" id="WC_ShipmentDisplay_links_2"><span><fmt:message key="BCT_SHOPPING_CART" bundle="${storeText}"/></span></a> |<c:if test="${empty validAddressId}"> <span class="on"><fmt:message key="BCT_ADDRESS" bundle="${storeText}"/></span> | <span><fmt:message key="TRA_SHIPPING" bundle="${storeText}"/></span> | <span><fmt:message key="TRA_BILLING" bundle="${storeText}"/></span> |</c:if> 

                     <c:if test="${!empty validAddressId}"><span class="on"><fmt:message key="BCT_SHIPPING" bundle="${storeText}"/></span></a> | <span><fmt:message key="BCT_BILLING" bundle="${storeText}"/></span> | </c:if> 
                     <span><fmt:message key="BCT_ORDER_SUMMARY" bundle="${storeText}"/></span>
		</c:if>
              </div>
        </div>
