		...
		***
		...
		<c:if test="${!empty param.thisCategoryID}">
				...
				<c:forEach var="discountEntry" items="${categoryDiscountDataBean.calculationCodeDataBeans}" varStatus="subCounter">
					...
					this.thisCategoryDiscount = '<p class="order_content">';
					this.thisCategoryDiscount += '<a class="discount" href="<c:out value="${DiscountDetailsDisplayViewURL}" />" id="WC_Link_CatDiscount_1_<c:out value="${counter.count}"/>">';
					this.thisCategoryDiscount += '<c:out value="${discountEntry.descriptionString}" escapeXml="true" /></a></p>';
		...
		<c:if test="${!empty param.someCategoryIDs}">
			<c:forTokens var="aCategoryId" delims=" ," items="${param.someCategoryIDs}">
				...
				<c:forEach var="discountEntry" items="${categoryDiscountDataBean.calculationCodeDataBeans}" varStatus="subCounter">
					...
					// Create discount text as a hyperlink
					this.categoryDiscount[<c:out value="${aCategoryId}"/>] += '<p class="order_content">';
					this.categoryDiscount[<c:out value="${aCategoryId}"/>] += '<a class="discount" href="<c:out value="${DiscountDetailsDisplayViewURL}" />" id="WC_Link_CatDiscount_2_<c:out value="${counter.count}"/>">';
					this.categoryDiscount[<c:out value="${aCategoryId}"/>] += '<c:out value="${discountEntry.descriptionString}" escapeXml="true" /></a></p>';
					this.foundDiscount = true;
				</c:forEach>
			</c:forTokens>
		</c:if>
				...
				<c:if test="${!empty productDiscounts}">
					<c:forEach var="discountEntry" items="${productDiscounts.calculationCodeDataBeans}" varStatus="discountCounter">
						...
						<c:url var="DiscountDetailsDisplayViewURL" value="DiscountDetailsDisplayView">
			    			...
			    			this.productDiscount[<c:out value="${productId}"/>] += '<div id="WC_ProductDiscount_div_<c:out value="${discountCounter.count}"/>" class="order_content">';
						this.productDiscount[<c:out value="${productId}"/>] += '<a href="<c:out value="${DiscountDetailsDisplayViewURL}" />" id="WC_Link_ProductDiscount_1_<c:out value="${discountCounter.count}"/>">';
			        		this.productDiscount[<c:out value="${productId}"/>] += '<c:out value="${discountEntry.descriptionString}" escapeXml="true" />';
						this.productDiscount[<c:out value="${productId}"/>] += '</a></div><br/>';
						this.foundDiscount = true;
					</c:forEach>
				</c:if>
			</c:forTokens>
				...
				<c:if test="${!empty itemDiscounts}">
					<c:forEach var="discountEntry" items="${itemDiscounts.calculationCodeDataBeans}" varStatus="discountCounter">
						...
						<c:url var="DiscountDetailsDisplayViewURL" value="DiscountDetailsDisplayView">
			    			...
			    			this.itemDiscount[<c:out value="${itemId}"/>] += '<p class="order_content">';
						this.itemDiscount[<c:out value="${itemId}"/>] += '<a href="<c:out value="${DiscountDetailsDisplayViewURL}" />" id="WC_Link_ProductDiscount_2_<c:out value="${discountCounter.count}"/>">';
			        		this.itemDiscount[<c:out value="${itemId}"/>] += '<c:out value="${discountEntry.descriptionString}" escapeXml="true" />';
						this.itemDiscount[<c:out value="${itemId}"/>] += '</a></p>';
						this.foundDiscount = true;
					</c:forEach>
				</c:if>
			</c:forTokens>
