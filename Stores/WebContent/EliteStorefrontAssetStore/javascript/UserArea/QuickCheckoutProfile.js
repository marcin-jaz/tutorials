...
QuickCheckoutProfile = {
	...
	/**
	 * This function updates the quickcheckout profile when 'AjaxMyAccount' option is enabled in the change flow option for the store.
	 * The function will refresh the profile display area using the given url after the update is successful.
	 * @param {string} formName The name of the form containing all the information required to create a quick address profile.
	 * @param {string} displayURL The URL used to refresh profile display.
	 */
	newUpdateProfile: function(formName, displayURL){
		var form = document.forms[formName];
		...
		wc.service.declare({
			id: "profileUpdate",
			actionId: "profileUpdate",
			formId: formName,
			successHandler: function(serviceResponse) {
				QuickCheckoutProfile.pageVar = 'quickcheckout';
				cursor_clear();
			},
			failureHandler: function(serviceResponse) {
				form.pay_temp_account.value = "";
		...
		if(this.validateForm(formName)) {
			...
			form.billing_addressType.value = "Billing";
			form.shipping_addressType.value = "Shipping";
			...
			cursor_wait();			
			wc.service.invoke("profileUpdate");
		}
	},
	...
	getCurrentProfile: function(addresses, prefix){
		...
		AddressHelper.loadStatesUI('QuickCheckout',prefix,prefix+'_stateDiv','_state1',true);
	},
