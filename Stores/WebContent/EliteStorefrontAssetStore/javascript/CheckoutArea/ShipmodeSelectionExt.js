	...
	*/
	submitAddressForm:function(formName, stateDivName, hasValidAddresses) {
		...
		if (hasValidAddresses) {
			//need to pass in orderItemId here for OrderChangeServiceItemUpdate so it will call DoInventoryActionCmd to get fulfillment center ID
			document.location.href = deletePaymentInstructionsURL + "OrderChangeServiceItemUpdate?remerge=***&check=*n&allocate=***&backorder=***&calculationUsage=-1,-2,-3,-4,-5,-6,-7&orderItemId_0="+this.orderItemId +"&URL=DOMOrderShippingBillingView?storeId="+this.storeId+"&catalogId="+this.catalogId+"&langId="+this.langId+"&payInStore="+payInStore;
