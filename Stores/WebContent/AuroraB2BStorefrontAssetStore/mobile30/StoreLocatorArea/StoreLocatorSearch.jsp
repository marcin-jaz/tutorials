...
<%
	request.setAttribute("escapedZipOrCity", SpecialCharacterHelper.escapeDollar(request.getParameter("zipOrCity")));
%>
...
<c:if test="${!empty zipOrCity}">
	<c:catch var="geoNodeException">
		<wcf:rest var="geoNodes" url="/store/{storeId}/geonode">
			...
			<wcf:param name="name" value="${escapedZipOrCity}"/>
	...
	<c:choose>
		<c:when test="${!empty zipOrCity && empty geoNodeException}">
			<c:set var="resultNum" value="${fn:length(geoNodes.recordSetTotal)}" />
