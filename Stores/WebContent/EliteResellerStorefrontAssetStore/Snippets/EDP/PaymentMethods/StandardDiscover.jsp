...
<div class="card_info" id="WC_StandardDiscover_div_1">
	<c:choose>
	...
	<input type="hidden" name="cc_brand" value="<c:out value="${cc_brand}" />" id="WC_StandardDiscover_inputs_1"/>
	<input type="hidden" name="paymentTCId" value="<c:out value="${paymentTCId}"/>" id="WC_StandardDiscover_inputs_2"/>
	...
	<span class="col1">
		<span class="card_number">
			<span>
				<label for="edp_Discover_account_<c:out value='${paymentAreaNumber}' />"><span class="required-field">*</span><fmt:message key="EDPPaymentMethods_VISA_CARD_NUMBER" bundle="${storeText}"/></label>
			</span>
			<span>
				<input type="text" style="width:142px"  name="account1" onchange="JavaScript:CheckoutPayments.removeCreditCardNumberAndCVV(<c:out value='${paymentAreaNumber}'/>, false, true);CheckoutHelperJS.paymentDataDirty(<c:out value='${paymentAreaNumber}'/>, true);CheckoutPayments.updatePaymentObject(<c:out value='${paymentAreaNumber}'/>, 'account');" value="<c:out value="${accountValue}"/>" id="account1_${paymentAreaNumber}" title="<fmt:message key="Checkout_ACCE_required" bundle="${storeText}"/> <fmt:message key="EDPPaymentMethods_VISA_CARD_NUMBER" bundle="${storeText}"/>" />
			</span>
		</span>	
		<span>
		...
		<span class="visaCVV2">
			<span id="CVVDetailsSection">					
				<span id="StandardDiscover_CVVDetails_${paymentAreaNumber}" class="order_link" tabindex="0"><fmt:message key="EDPPaymentMethods_CVV_WHAT_IS" bundle="${storeText}"/></span>
				<span id="StandardDiscover_CVVDetails_${paymentAreaNumber}_span_2" dojoType="wc.widget.Tooltip" connectId="StandardDiscover_CVVDetails_${paymentAreaNumber}" style="display: none;">
					<div id="tooltip_popup">
						<div id="WC_StandardDiscover_div_2_${paymentAreaNumber}" class="header">
							<div id="WC_StandardDiscover_div_3_${paymentAreaNumber}" class="close">
								<fmt:message key="EDPPaymentMethods_CVV_WHAT_IS" bundle="${storeText}"/>
							</div>
						</div>
						<div id="WC_StandardDiscover_div_4_${paymentAreaNumber}" class="bodyarea">
							<div id="WC_StandardDiscover_div_5_${paymentAreaNumber}" class="bodycontent">
								<fmt:message key="EDPPaymentMethods_CVV_DEFINITION" bundle="${storeText}"/>
							</div>
						</div>
						<div id="WC_StandardDiscover_div_6_${paymentAreaNumber}" class="footer"></div>
				...
				<script type="text/javascript">dojo.addOnLoad(function() {parseWidget("StandardDiscover_CVVDetails_${paymentAreaNumber}_span_2"); });</script>
			</span>
		</span>
		<span class="date">
