...
SKUListJS = {
	...
	*/
	populateOnlineInvDetails: function(serviceResponse, ioArgs) {
		if(serviceResponse.onlineInventory){			
			...
			var fullInvCells = dojo.query(".OnlineAvailability.gridCell", listTable);
	...
	**/
	showTable: function(productId, top_category, parent_category_rn, categoryId, widgetPrefix) {
		...
		if (skuListWidget) {
			cursor_wait();
			skuListWidget.refresh({"storeId":this.storeId, "productId":productId, "disableProductCompare":this.disableProductCompare, "jspStoreImgDir":imageDirectoryPath, "top_category":top_category, "parent_category_rn":parent_category_rn, "categoryId":categoryId, "widgetPrefix":widgetPrefix});
		}
	...
	**/
	...
	arrangeProductDetailTable: function(productId) {
		...
		if (dojo.query(".columnHeader.unanchored.Compare", listTable).length != 0) {
		...
		for (var i = 0; i < headersToMove.length; i++) {
			...
			if (dojo.indexOf(unanchoredHeaders, headersToMove[i]) != -1) {
				...
				var className = dojo.attr(headersToMove[i], "class").split(" ")[0];

				// Hide the main column for this CSS class
				var columnToHide = dojo.query(".columnHeader.unanchored." + className + ", .gridCell.unanchored." + className, listTable);
				columnToHide.style('display', 'none');

				// Show the attribute in the expanded content section
				var expandedColumnsToShow = dojo.query(".expandedCol." + className, listTable);
