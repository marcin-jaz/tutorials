...
<c:if test="${env_inPreview && !env_storePreviewLink && ( empty ignorePreview )}">
  ...
  <!-- Special handling to exclude the rich text widget -->
	...
	<%-- refresh area for eSpost 'SearchResults_Content' preview popup only --%>
		...
		<div dojoType="wc.widget.RefreshArea" id="SearchRuleInfo${widgetSuffix}" objectId="${widgetSuffix}" controllerId="searchRuleInfo_controller${widgetSuffix}" ariaMessage="<fmt:message key='ACCE_STATUS_SEARCHRULEINFORMATION_UPDATED' bundle='${widgetText}' />" ariaLiveId="${ariaMessageNode}" role="region" aria-label="<fmt:message key='SEARCH_RULE_INFO_PREVIEW_POPUP_ACCE_Label' bundle='${widgetText}' />">
	</c:if>
