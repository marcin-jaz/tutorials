...
<c:if test="${!empty uniqueID}">
	...
	<div class="header_bar">
		<div class="title"><fmt:message key="PT_PRODUCT_ATTACHMENT" bundle="${widgetText}"/> 
	...
	<c:if test="${not empty attachmentGrp}">
		<c:forEach var="attachmentsList" items="${attachmentGrp}">
			<fmt:message key='PT_PRODUCT_ATTACHMENT_USAGE_USERMANUAL' bundle='${widgetText}' var="userManualText"/>
			<div class="header">${(attachmentsList.key=="USERMANUAL")?userManualText:fn:toLowerCase(attachmentsList.key)}</div>
					...
					<div class="attachment">
						...
						<c:choose>
							<c:when test="${(mimePart eq 'image') || (mimePart eq 'images')|| (mimePart eq 'application') || (mimePart eq 'applications') || ( mimePart eq 'text') 							
											...
											|| (mimePart eq 'model')}"> 
										<div class="icon">
											<a href="${http}${attachmentDetails['assetPath']}" target="_new" aria-describedby="acce_attachment_${fn:toLowerCase(attachmentsList.key)}_${status.count}" aria-label="<c:out value="${(empty attachmentDetails['name'])?docEmpty:attachmentDetails['name']}"/>" id="WC_TechnicalSpecification_Image_2_${status.count}">
										...
										<div class="description">
											<fmt:message key='PT_PRODUCT_ATTACHMENT_EMPTY_NAME' bundle='${widgetText}' var="docEmpty"/>
											<a href="${attachmentDetails['assetPath']}" tabIndex="-1" aria-hidden="true" target="_blank" id="WC_TechnicalSpecification_Links_1_${status.count}">
												<c:out value="${(empty attachmentDetails['name'])?docEmpty:attachmentDetails['name']}"/>
											</a>
