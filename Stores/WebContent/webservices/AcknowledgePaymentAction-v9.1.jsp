...
//*
%>
<%-- response builder for the AcknowledgePaymentAction BOD --%>
...
%>
<wc:AcknowledgePaymentAction releaseID="9.0" versionID="9.1" xmlns:oa="http://www.openapplications.org/oagis/9" xmlns:wc="http://www.ibm.com/xmlns/prod/WebSphereCommerce">
	<oa:ApplicationArea>
	...
	<wc:DataArea>
		<oa:Acknowledge />
		<c:forEach var="result" items="${RequestProperties.results}">
			<wc:PaymentAction>
				<wc:Action><c:out value="${result.action}" /></wc:Action>
				<c:if test="${!empty result.result.reasonCode}">
					<wc:ReasonCode><c:out value="${result.result.reasonCode}" /></wc:ReasonCode>
				</c:if>
				<wc:Status><c:out value="${result.result.status}" /></wc:Status>
				<c:set var="credit" value="${result.result.credit}" />
				<c:set var="payment" value="${result.result.payment}" />
				<c:set var="paymentInstruction" value="${result.result.paymentInstruction}" />
				<wc:PaymentEntity>
