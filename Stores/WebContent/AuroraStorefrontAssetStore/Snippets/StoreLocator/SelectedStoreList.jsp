...
<table id="bopis_table1" tabindex="-1" summary="<fmt:message bundle="${storeText}" key='SELECTED_STORES_SUMMARY'/>" cellpadding="0" cellspacing="0" border="0" width="100%">
  ...
  <tr id="${physicalStores.PhysicalStore[i].uniqueID}">
    ...
    <td id="WC_SelectedStoreList_td_1_<c:out value='${status.count}'/>" width="23" headers="PhysicalStores_tableCell_0" <c:choose><c:when test="${i == resultNum - 1}"><c:out value="class=no_bottom_border"/></c:when><c:otherwise><c:out value="class=dotted_bottom_border"/></c:otherwise></c:choose>>
      ...
      <input name="pickUpStore" id="pickUpStore_${i}" type="radio" onclick="Javascript:PhysicalStoreCookieJS.setPickUpStoreIdToCookie(${physicalStores.PhysicalStore[i].uniqueID}); pickUpStoreId=${physicalStores.PhysicalStore[i].uniqueID};" checked="checked" />
              </c:when>
              <c:otherwise>
      <input name="pickUpStore" id="pickUpStore_${i}" type="radio" onclick="Javascript:PhysicalStoreCookieJS.setPickUpStoreIdToCookie(${physicalStores.PhysicalStore[i].uniqueID}); pickUpStoreId=${physicalStores.PhysicalStore[i].uniqueID};" />
              </c:otherwise>
            </c:choose>
          </c:when>
          <c:otherwise>
    ...
    <td id="WC_SelectedStoreList_td_6_<c:out value='${status.count}'/>" headers="PhysicalStores_tableCell_4" <c:choose><c:when test="${i == resultNum - 1}"><c:out value="class=no_bottom_border"/></c:when><c:otherwise><c:out value="class=dotted_bottom_border"/></c:otherwise></c:choose>>
      <a class="remove_store_link hover_underline tlignore" id="removePhysicalStoreAction<c:out value='${i}' />" href="Javascript:setCurrentId('removePhysicalStoreAction${i}'); storeLocatorJS.removePhysicalStore(<c:out value="${physicalStores.PhysicalStore[i].uniqueID}" />); storeLocatorJS.removeFromStoreList('<c:out value="${fromPage}" />','<c:out value="${physicalStores.PhysicalStore[i].uniqueID}" />');"><img src="<c:out value='${jspStoreImgDir}${env_vfileColor}' />table_x_delete.png" alt="<fmt:message bundle="${storeText}" key='TABLE_X_DELETE_IMAGE'/>" /><fmt:message bundle="${storeText}" key="REMOVE_PHYSICAL_STORE" /></a>
    </td>
  </tr>
