						...
						<c:choose>
							<c:when test="${payment.paymentMethodName eq paymentMethodSelected}">
								<c:forEach var="addressInPayment" items="${payment.usableBillingAddress}">
									...
									<c:when test="${addressInPayment.nickName eq  profileBillingNickname}"><fmt:message bundle="${storeText}" key="QC_DEFAULT_BILLING" /></c:when><c:otherwise>${addressInPayment.nickName}</c:otherwise></c:choose></option>
