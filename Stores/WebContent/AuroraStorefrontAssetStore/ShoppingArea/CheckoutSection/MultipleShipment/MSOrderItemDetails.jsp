		...
		<span class="text">
			...
			<span class="paging">
				...
				<c:if test="${beginIndex != 0}">
					...
					wc.render.updateContext('multipleShipmentDetailsContext',{'beginIndex':'<c:out value='${beginIndex - pageSize}'/>','pageSize':'<c:out value='${pageSize}'/>', 'initializeJS':'true'});}}">
				</c:if>
...
<table id="order_details" cellpadding="0" cellspacing="0" border="0" width="100%" summary="<fmt:message bundle="${storeText}" key="SHOPCART_TABLE_SUMMARY"  />">
	...
	<c:forEach var="orderItem" items="${pgorder.orderItem}" varStatus="status">
		...
		<tr>
			 <th class="th_align_left_normal <c:out value="${nobottom}"/>" id="MultipleShipping_rowHeader_product<c:out value='${status.count}'/>" abbr="<fmt:message bundle="${storeText}" key="Checkout_ACCE_for"  /> ${catEntry.name}" width="225">
			...
			<c:forEach var="discountsIterator" items="${discountReferences}" varStatus="status2">
				...
				<c:if test="${discounts.displayLevel == 'OrderItem'}">
					<tr>
						...
						<td colspan="5" class="th_align_left_dotted_top_solid_bottom" abbr="<fmt:message bundle="${storeText}" key="Checkout_ACCE_prod_discount"  /> ${catEntry.name}" id="MultipleShipment_rowHeader_discount<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>">
...
<flow:ifDisabled feature="AjaxCheckout">
	...
	<%out.flush();%>
		<c:import url="${env_jspStoreDir}/ShoppingArea/CheckoutSection/MultipleShipment/ShippingAddressDetailsInShort.jsp">
